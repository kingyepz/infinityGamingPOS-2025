...
const paymentFormSchema = z.object({
  ...
  if (data.paymentMethod === 'mpesa') {
    return !!data.mpesaReference && data.mpesaReference.length > 3;
  }
  return true;
}, {
...
export default function EndSessionDialog({ isOpen, onClose, session, onProcessPayment }: EndSessionDialogProps) {
  ...
  const handleSubmit = (data: PaymentFormData) => {
    const paidSession: GameSession = {
      ...
      mpesaReference: data.paymentMethod === 'mpesa' ? data.mpesaReference : undefined,
    };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-3 py-4 text-sm">
          <div className="flex justify-between"><span>Start Time:</span> <span>{format(session.startTime, 'p')}</span></div>
          {session.endTime && <div className="flex justify-between"><span>End Time:</span> <span>{format(session.endTime, 'p')}</span></div>}
          ...
          <div className="flex justify-between"><span>Points Earned:</span> <span className="font-medium">{session.pointsAwarded || 0} pts</span></div>
        </div>
                  ...
                  <FormControl>
                    ...
                    >
                      ...
                      <FormItem className="flex items-center space-x-2 space-y-0">
                        ...
                        <FormLabel className="font-normal">MPesa</FormLabel>
                      </FormItem>
                    </RadioGroup>
                  </FormControl>
                  <FormMessage />
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="e.g. RKT123ABC45" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
