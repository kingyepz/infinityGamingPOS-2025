...
export default function SessionsPage() {
  ...
  useEffect(() => {
    ...
    setCustomers([
      { id: 'cust1', name: 'John Doe', phone: '0712345678', email: 'john.doe@example.com', loyaltyPoints: 120, createdAt: new Date() },
      { id: 'cust2', name: 'Jane Smith', phone: '0723456789', email: 'jane.smith@example.com', loyaltyPoints: 75, createdAt: new Date() },
      { id: 'cust3', name: 'Alex Green', phone: '0734567890', email: 'alex.green@example.com', loyaltyPoints: 200, createdAt: new Date() },
    ]);
  ...
  const handleStartSession = (data: Omit<GameSession, 'id' | 'startTime' | 'paymentStatus' | 'createdAt' | 'customerName' | 'consoleName'> & { customerId: string; consoleId: string; }) => {
  ...
  const handleOpenEndSessionDialog = (session: GameSession) => {
    ...
    if (session.billingType === 'per-hour' && session.rate) {
      subtotal = Math.ceil(durationMinutes / 60) * session.rate; // Bill per hour block
  ...
  const handleProcessPayment = useCallback((paidSession: GameSession) => {
    ...
    setCustomers(prev => prev.map(cust => 
      ...
      ? { ...cust, loyaltyPoints: cust.loyaltyPoints + (paidSession.pointsAwarded || 0) } 
      : cust
    ));
